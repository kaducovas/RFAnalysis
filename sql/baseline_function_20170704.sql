-- Function: umts_baseline.baseline_audit(text)

-- DROP FUNCTION umts_baseline.baseline_audit(text);
...
--				if(cur_row_par.configured in ('-16','-18')) then
					...
					r.status := 'NOK';
					r.mml = concat(replace(replace(replace(replace(replace(replace(cur_row.mml,'%RNCID%',cur_row_par.element_id),
					'%CELLID%',cur_row_par.element_id),
					'%NCELLRNCID%',cur_row_par.element_id),
					'%NCELLID%',cur_row_par.element_id),
					'%IDLEQOFFSET1SN%',cur_row_par.element_id),
					'%IDLEQOFFSET2SN%',cur_row_par.element_id),
					'{',cur_row_par.node,'}');
--					r.mml = concat(replace(cur_row.mml,'%CELLID%',cur_row_par.element_id),'{',cur_row_par.node,'}');
				end if;
			end if;
