...
or m.rtwp > -90 
or (b.status = 'NOK')) then 'NOK' else 'OK' end otm,
e.ee as ee,
null as load,
 u.code_utilization, 
...
left join w30_ee e on n.rnc = e.rnc and n.cellid = e.cellid
...
left join (select *,get_nodeb_name(node, 'nodeb') as nodeb_norm  from w30) t on n.nodeb = t.nodeb_norm and n.year = t.year and n.week = t.week
...
from umts_baseline.consistency_check_hist where status = 'NOK' and mo not in ('UCELLLICENSE') and baseline_date = '2017-07-21'
) t
group by node,element_id,status) b on n.rnc=b.node and n.cellid::text = b.element_id
  where n.year = 2017 and n.week = 30) t1

) to '/home/postgres/dump/w30_triagev.csv' delimiter ';' csv header