...
-- -- e
-- -- -- -- W__
	LEFT JOIN w38_ee e on n.rnc = e.rnc AND n.cellid = e.cellid
	LEFT JOIN w38_load_ee_v4 load_ee on n.rnc = load_ee.rnc AND n.cellid = load_ee.cellid
-- -- o
...
-- -- t
-- -- -- -- W__
	LEFT JOIN (SELECT *,get_nodeb_name(node, 'nodeb') AS nodeb_norm  FROM w38_tx) t ON n.nodeb = t.nodeb_norm AND n.year = t.year AND n.week = t.week
-- -- i
...
-- -- t
	...
	FROM umts_baseline.consistency_check_hist WHERE status = 'NOK' AND mo NOT IN ('UCELLLICENSE') AND baseline_date IN (SELECT MAX(baseline_date) FROM umts_baseline.consistency_check_hist)) t
-- -- t1
-- -- -- -- W__
	GROUP BY node,element_id,status) b ON n.rnc=b.node AND n.cellid::TEXT = b.element_id WHERE n.year = 2017 AND n.week = 38) t1

-- SAVE AT/SAVE AS:
-- -- -- -- W__
)	TO '/home/postgres/dump/w38_triage.csv' DELIMITER ';' CSV header