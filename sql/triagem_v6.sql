...
case when (qda_ps_f2h >= 90 and qdr_ps >=95  and retention_ps = 100 and n.availability>=99.5 and qda_cs >=92 and qdr_cs>=92 and m.rtwp <=-90 and retention_cs >=99.5 and hsdpa_users_ratio>99.5 and throughput < 400) then 'OK' else 'NOK' end as kpi,  
i.wbbp_total,
...
left join w19_ee e on n.rnc = e.rnc and n.cellid = e.cellid
...
group by node,element_id,status) b on n.rnc=b.node and n.cellid::text = b.element_id
  where n.year = 2017 and n.week = 19) t1
  
  

) to '/home/postgres/dump/w19_triage.csv' delimiter ';' csv header		
