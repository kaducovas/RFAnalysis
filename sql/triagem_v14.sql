...
-- -- Cell Info
	FROM (SELECT	n.year, n.week, n.region, n.rnc, n.nodebname, n.node, n.cellid,
...
-- -- KPI
    ...
    CHAR_LENGTH(
    	CONCAT(
            CASE WHEN qda_cs<92 THEN '1' ELSE '' END,
            CASE WHEN qda_ps_f2h<90 THEN '1' ELSE '' END,
            CASE WHEN qdr_cs<92 THEN '1' ELSE '' END,
            CASE WHEN qdr_ps<95 THEN '1' ELSE '' END,
            CASE WHEN throughput<92 THEN '1' ELSE '' END,
            CASE WHEN n.availability<99.5 THEN '1' ELSE '' END,
            CASE WHEN retention_cs<99.5 THEN '1' ELSE '' END,
            CASE WHEN retention_ps<100 THEN '1' ELSE '' END,
            CASE WHEN hsdpa_users_ratio<99.5 THEN '1' ELSE '' END,
            CASE WHEN m.rtwp>-90 THEN '1' ELSE '' END
		)
	) AS kpis_out,
...
-- -- e
-- -- -- -- W__
	LEFT JOIN w40_ee e on n.rnc = e.rnc AND n.cellid = e.cellid
	LEFT JOIN w40_load_ee_v4 load_ee on n.rnc = load_ee.rnc AND n.cellid = load_ee.cellid
-- -- o
...
-- -- t
-- -- -- -- W__
	LEFT JOIN (SELECT *,get_nodeb_name(node, 'nodeb') AS nodeb_norm  FROM w40_tx) t ON n.nodeb = t.nodeb_norm AND n.year = t.year AND n.week = t.week
-- -- i
	--LEFT JOIN (SELECT DISTINCT ON (nodebname_normalized) * FROM umts_inventory.board_status_gexport) i ON n.nodeb = i.nodebname_normalized -- .m2k -> .gexport
   	LEFT JOIN (SELECT DISTINCT ON (nodebname_normalized) * FROM umts_inventory.board_status_gexport ORDER BY nodebname_normalized, status DESC, CHAR_LENGTH(board_found) DESC, boardnum DESC) i ON n.nodeb = i.nodebname_normalized
-- -- alarm
...
-- -- t
	...
	FROM umts_baseline.consistency_check_nok_hist WHERE status = 'NOK' AND mo NOT IN ('UCELLLICENSE') AND baseline_date IN (SELECT MAX(baseline_date) FROM umts_baseline.consistency_check_nok_hist)) t --umts_baseline.consistency_check_hist -> umts_baseline.consistency_check_nok_hist
-- -- t1
-- -- -- -- W__
	GROUP BY node,element_id,status) b ON n.rnc=b.node AND n.cellid::TEXT = b.element_id WHERE n.year = 2017 AND n.week = 40) t1

-- SAVE AT/SAVE AS:
-- -- -- -- W__
)	TO '/home/postgres/dump/w40_triage.csv' DELIMITER ';' CSV header