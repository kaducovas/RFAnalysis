copy (SELECT n.year, n.week, n.region, n.rnc, n.nodeb,n.nodebname, n.node, n.cellid,n.node, qda_cs, qda_ps, 
...
CASE when qda_ps_f2h >= 90 then 'OK' ELSE 'NOK' end as status_qda_ps_f2h,
...
CASE when nqi_ps_f2h>=70 then 'OK' ELSE 'NOK' end as status_nqi_ps_f2h,
CASE when qda_cs >=92 then 'OK' ELSE 'NOK' end as status_qda_cs,
CASE when qdr_cs>=92 then 'OK' ELSE 'NOK' end as status_qdr_cs,
CASE when nqi_cs>=70 then 'OK' ELSE 'NOK' end as status_nqi_cs,
...
case when (t.status = 'Cong. Rota' or t.status = 'Expansão IP' or t.status = 'Expansão ATM') then 'NOK' else 'OK' end tx
...
and n.region = 'CO') to '/home/postgres/dump/triagem_co.csv' delimiter ';' csv header		