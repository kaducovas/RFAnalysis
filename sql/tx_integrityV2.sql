...
FROM
	(SELECT tx_ip.rnc,tx_ip.ani,tx_ip.nodet,tx_ip.node,tx_ip.transt,tx_ip.datetime,tx_ip.ping_meanlost,tx_ip.ping_meanjitter,tx_ip.ping_meandelay,
...
atm_dl_utilization,atm_ul_utilization,			
                CASE
                    WHEN tx_ip.ping_meandelay > 80::double precision OR tx_ip.ping_meanjitter > 2::real OR tx_ip.ping_meanlost > 5::real or atm_dl_utilization >80 or atm_ul_utilization >80 THEN 0
                    ELSE 1
                END AS user_good_tx,
                CASE
                    WHEN tx_ip.ping_meandelay > 80::double precision OR tx_ip.ping_meanjitter > 2::real OR tx_ip.ping_meanlost > 5::real or atm_dl_utilization >80 or atm_ul_utilization >80 THEN 1
                    ELSE 0
                END AS user_bad_tx
                from
(select
...
from
(select *,get_nodeb_name(nodeb,'nodeb') as norm from umts_kpi.tx_atm) atm 
left join 
...
on ip.rnc = adj.rncname and adj.ani = ip.ani) tx_ip
on tx_atm.rnc = tx_ip.rnc and tx_atm.ani = tx_ip.ani
WHERE tx_ip.datetime::time without time zone >= '07:00:00'::time without time zone AND tx_ip.datetime::time without time zone <= '23:59:00'::time without time zone) t
GROUP BY t.rnc, t.ani, t.nodet, t.node, date_part('year'::text, t.datetime::date), date_part('week'::text, t.datetime::date + '1 day'::interval),t.transt
