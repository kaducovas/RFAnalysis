...
-- -- e
-- -- -- -- W__
	LEFT JOIN w39_ee e on n.rnc = e.rnc AND n.cellid = e.cellid
	LEFT JOIN w39_load_ee_v4 load_ee on n.rnc = load_ee.rnc AND n.cellid = load_ee.cellid
-- -- o
...
-- -- t
-- -- -- -- W__
	LEFT JOIN (SELECT *,get_nodeb_name(node, 'nodeb') AS nodeb_norm  FROM w39_tx) t ON n.nodeb = t.nodeb_norm AND n.year = t.year AND n.week = t.week
-- -- i
...
-- -- t1
-- -- -- -- W__
	GROUP BY node,element_id,status) b ON n.rnc=b.node AND n.cellid::TEXT = b.element_id WHERE n.year = 2017 AND n.week = 39) t1

-- SAVE AT/SAVE AS:
-- -- -- -- W__
)	TO '/home/postgres/dump/w39_triage_kpis_out.csv' DELIMITER ';' CSV header