COPY (SELECT a.year, b.week, a.region, a.cidade, a.frequency, a.nqi, b.cqi, b.downlink_avg_thp, b.uplink_avg_thp,"downlink_traffic_volume (GB)", b.average_user_volume,retention_4g 
FROM
 (SELECT date_part('year'::text, nqi_daily.date) AS year,
  ...
  GROUP BY date_part('week'::text, nqi_daily.date + '1 day'::interval), date_part('year'::text, nqi_daily.date), cells.region,frequency,cidade,uf) a
  ...
  (SELECT date_part('year'::text, main_kpis_lte_daily.date) AS year,
   ...
   FROM lte_kpi.main_kpis_lte_daily
     ...
     where cidade in ('RECIFE','CURITIBA','FLORIANÓPOLIS','BELO HORIZONTE','SALVADOR') and date > '2017-07-30'
  GROUP BY date_part('week'::text, main_kpis_lte_daily.date + '1 day'::interval), date_part('year'::text, main_kpis_lte_daily.date), cells.region,frequency,cidade,uf) b

ON a.year = b.year AND a.week = b.week AND a.frequency = b.frequency AND a.cidade = b.cidade)
TO '/home/postgres/dump/lte_cidade_freq_kpis.csv' with csv
	header delimiter ';'