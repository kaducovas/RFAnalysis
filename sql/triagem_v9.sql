...
-- -- OTM
	...
	b.mo_out AS mo_out,	
	CASE 	WHEN	((e.ee > 22 AND e.balanced = 'NOK') OR (o.count > 0 AND o.count IS NOT NULL)
			OR m.rtwp > -90 
...
-- -- CAPACITY
	...
	CASE 	WHEN	(u.code_utilization >= 70::REAL OR u.dlpower_utilization >= 70::REAL OR u.user_fach_utilization >= 70::REAL
			...
			OR u.dlce_utilization >= 70::REAL OR u.ulce_utilization >= 70::REAL) THEN 'NOK' ELSE 'OK'
	END AS capacity,
...
-- -- e
-- -- -- -- W__
	LEFT JOIN w32_ee e on n.rnc = e.rnc AND n.cellid = e.cellid
-- -- o
...
-- -- t
-- -- -- -- W__
	LEFT JOIN (SELECT *,get_nodeb_name(node, 'nodeb') AS nodeb_norm  FROM w32_tx) t ON n.nodeb = t.nodeb_norm AND n.year = t.year AND n.week = t.week
-- -- i
...
-- -- t
	...
	FROM umts_baseline.consistency_check_hist WHERE status = 'NOK' AND mo NOT IN ('UCELLLICENSE') AND baseline_date = '2017-07-21') t
-- -- t1
-- -- -- -- W__
	GROUP BY node,element_id,status) b ON n.rnc=b.node AND n.cellid::TEXT = b.element_id WHERE n.year = 2017 AND n.week = 32) t1

-- SAVE AT/SAVE AS:
-- -- -- -- W__
)	TO '/home/postgres/dump/w32_triage.csv' DELIMITER ';' CSV header